...
class Prompt extends Command
{
    ...
    public function handle()
    {
        ...
        $book_users = $this->repository->getData($data, 'book');
        $book_users->each(function($items, $key) {
            $this->repository->createPrompt($key, $key, config('model.notification.owner_prompt'));
            foreach ($items as $item) {
                $this->repository->createPrompt($item->owner_id, $item->user_id, config('model.notification.user_prompt'), $item->book->title, $item->book_id);
            }
        });
    }
}
